...
def chals():
    ...
    if can_view_challenges():
        teamid = session['id']
        chals = Challenges.query.filter(or_(Challenges.hidden != True, Challenges.hidden == None)).add_columns('id', 'name', 'value', 'description', 'category', 'dependsupon').order_by(Challenges.value).all() # krw: want to filter these for challenges have either an empty 'dependsupon' or where 'dependsupon' is in our list of solved challenges
        solves = Solves.query.filter_by(teamid=teamid).all() #Solves.query.join(Teams, Solves.teamid == Teams.id).filter(Solves.teamid == session['id'], Teams.banned == False).all()
